...
class wafv2_global_web_acls_logging_enabled(Check):
    def execute(self):
        findings = []
        for global_web_acl in wafv2_client.global_web_acls:
            ...
            if global_web_acl.logging_enabled:
                report.status = "PASS"
                report.status_extended = (
                    f"Global AWS WAFv2 Web ACL {global_web_acl.id} has logging enabled."
                )
            else:
                report.status = "FAIL"
                report.status_extended = (
                    f"Global AWS WAFv2 Web ACL {global_web_acl.id} does not have logging enabled."
                )
